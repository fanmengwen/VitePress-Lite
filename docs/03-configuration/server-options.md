...
请注意，如果使用了非相对的 [基础路径 `base`](/config/shared-options#base)，则必须在每个 key 值前加上该 `base`。
...
此黑名单不适用于[公共目录](/guide/assets#the-public-directory)。公共目录中的所有文件均未经任何过滤，因为它们会在构建过程中直接复制到输出目录。
