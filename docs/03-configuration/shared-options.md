...
date: "2025-11-16"
published: true
excerpt: "除非另有说明，本节中的选项适用于所有开发、构建和预览。"
---
...
*   **类型：** `string`
*   **默认：** `/`
*   **相关：** [`server.origin`](/config/server-options#server-origin)
...
如果你已经为 [SSR 外部化的依赖](/guide/ssr#ssr-externals) 配置了别名，你可能想要为真实的 `node_modules` 包配别名。[Yarn](https://classic.yarnpkg.com/en/docs/cli/add/#toc-yarn-add-alias) 和 [pnpm](https://pnpm.io/aliases/) 都支持通过 `npm:` 前缀配置别名。
...
## resolve.conditions [non-inherit](/guide/api-environment#environments-configuration) [​](#resolve-conditions)
...
## resolve.mainFields [non-inherit](/guide/api-environment#environments-configuration) [​](#resolve-mainfields)
...
当使用 [Lightning CSS](./../guide/features#lightning-css) 时，该选项不会产生任何效果。如果要启用该选项，则应该使用 [`css.lightningcss.cssModules`](https://lightningcss.dev/css-modules.html) 来替代。
...
该选项用于选择用于 CSS 处理的引擎。详细信息请查看 [Lightning CSS](./../guide/features#lightning-css)。
...
当 [`build.minify`](./build-options#build-minify) 为 `true` 时，所有最小化的优化过程都会被默认应用，要禁用它的 [某些特定方面](https://esbuild.github.io/api/#minify)，请设置 `esbuild.minifyIdentifiers`、`esbuild.minifySyntax` 或 `esbuild.minifyWhitespace` 三种选项其中任意一种为 `false`。注意 `esbuild.minify` 选项无法用于覆盖 `build.minify`。
