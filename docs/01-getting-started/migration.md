...
*   [\[#20222\] feat: apply some middlewares before `configureServer` hook](https://github.com/vitejs/vite/pull/20222), [\[#20224\] feat: apply some middlewares before `configurePreviewServer` hook](https://github.com/vitejs/vite/pull/20224)
    *   某些中间件现在会在 `configureServer` / `configurePreviewServer` 钩子之前被应用。请注意，如果你不希望某个路由应用 [`server.cors`](./../config/server-options#server-cors) / [`preview.cors`](./../config/preview-options#preview-cors) 配置，请务必从响应中移除相关的请求头。

## 从 v5 迁移 [​](#migration-from-v5)

请先查阅 Vite v6 文档中的 [从 v5 迁移指南](https://v6.vite.dev/guide/migration.html)（[中文版](/guide/migration-from-v5)），了解如何将你的应用迁移到 Vite 6 所需的变更，然后再继续执行本页中的相关更改。
