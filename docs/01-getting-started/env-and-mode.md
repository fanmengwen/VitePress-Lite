...
*   **`import.meta.env.BASE_URL`**: {string} 部署应用时的基本 URL。他由[`base` 配置项](/config/shared-options#base)决定。
    
*   **`import.meta.env.PROD`**: {boolean} 应用是否运行在生产环境（使用 `NODE_ENV='production'` 运行开发服务器或构建应用时使用 `NODE_ENV='production'` ）。
    
*   **`import.meta.env.DEV`**: {boolean} 应用是否运行在开发环境 (永远与 `import.meta.env.PROD`相反)。
    
*   **`import.meta.env.SSR`**: {boolean} 应用是否运行在 [server](./ssr#conditional-logic) 上。
...
Vite 使用 [dotenv](https://github.com/motdotla/dotenv) 从你的 [环境目录](/config/shared-options#envdir) 中的下列文件加载额外的环境变量：
...
Vite 支持以相反的顺序扩展变量。 例如，下面的 `.env` 将被评估为 `VITE_FOO=foobar`, `VITE_BAR=bar`。
