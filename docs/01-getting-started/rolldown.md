...
date: "2025-11-16"
published: true
excerpt: "Vite 计划将由 Rust 驱动的 JavaScript 打包工具 Rolldown 集成进来，以提升构建的性能和功能。"
---
...
`@vitejs/plugin-react-oxc` 插件已被弃用
...
插件作者可以选择使用 [钩子过滤功能](https://rolldown.rs/apis/plugin-hook-filters)，以减少 Rust 和 JavaScript 运行时之间的通信开销。 但如果你使用的某些插件还未采用该功能，而你又希望受益于它，可以使用 `withFilter` 包装器自行为插件添加过滤条件。
...
Rolldown-Vite 有一个兼容层，用于将 `esbuild` 的选项转换为相应的 Oxc 或 `rolldown` 选项。正如 [生态系统 CI](https://github.com/vitejs/vite-ecosystem-ci/blob/rolldown-vite/README-temp.md) 中测试的那样，这在许多情况，包括简单的 `esbuild` 插件下都有效。 虽说如此，但 **我们将在未来移除对 `esbuild` 选项的支持**，并鼓励你尝试使用相应的 Oxc 或 `rolldown` 选项。 你可以从 `configResolved` 钩子获取由兼容层设置的选项。
