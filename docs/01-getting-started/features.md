...
date: "2025-11-16"
published: true
excerpt: "对非常基础的使用来说，使用 Vite 开发和使用一个静态文件服务器并没有太大区别。"
---
...
如果你使用自己的框架运行 JSX，可以使用 [`esbuild` 选项](/config/shared-options#esbuild) 来配置自定义的 `jsxFactory` 和 `jsxFragment`。例如，Preact 插件会使用：
...
请注意，CSS 最小化压缩将在 PostCSS 之后运行，并会使用 [`build.cssTarget`](/config/build-options#build-csstarget) 选项。
...
CSS modules 行为可以通过 [`css.modules` 选项](/config/shared-options#css-modules) 进行配置。
...
从 Vite 4.4 开始，已经实验性地支持 [Lightning CSS](https://lightningcss.dev/)。可以通过在配置文件中添加 [`css.transformer: 'lightningcss'`](./../config/shared-options#css-transformer) 并安装可选的 [`lightningcss`](https://www.npmjs.com/package/lightningcss) 依赖项来选择使用它：
...
如果启用，CSS 文件将由 Lightning CSS 处理，而不是 PostCSS。可以将 Lightning CSS 的选项传递给 [`css.lightningcss`](./../config/shared-options#css-lightningcss) 选项来配置。

要配置 CSS Modules，需要使用 [`css.lightningcss.cssModules`](https://lightningcss.dev/css-modules.html) 而不是 [`css.modules`](./../config/shared-options#css-modules)（后者是用于配置 PostCSS 处理 CSS Modules 的方式）。

默认情况下，Vite 使用 esbuild 来压缩 CSS。通过 [`build.cssMinify: 'lightningcss'`](./../config/build-options#build-cssminify) 进行配置，也可以将 Lightning CSS 用作 CSS 最小化压缩。
...
*   该 Glob 模式会被当成导入标识符：必须是相对路径（以 `./` 开头）或绝对路径（以 `/` 开头，相对于项目根目录解析）或一个别名路径（请看 [`resolve.alias` 选项](/config/shared-options#resolve-alias))。
...
预编译的 `.wasm` 文件可以通过 `?init` 来导入。 默认导出一个初始化函数，返回值为所导出 [`WebAssembly.Instance`](https://developer.mozilla.org/en-US/docs/WebAssembly/JavaScript_interface/Instance) 实例对象的 Promise：
...
在 SSR 中，作为 `?init` 导入的 `fetch()` 可能会失败，导致 `TypeError: Invalid URL` 报错。 请参见问题 [在 SSR 中支持 wasm](https://github.com/vitejs/vite/issues/8882)。
...
关于如何配置打包全部 worker，可以查看 [Worker 选项](/config/worker-options) 了解更多相关细节。
...
Vite 可以通过 [`build.license`](/config/build-options#build-license) 选项生成一个包含构建中使用的所有依赖项许可证的文件。该文件可以被托管，用于显示和确认应用程序所使用的依赖项。
...
要将文件服务于不同的路径，你可以传递 `{ fileName: 'license.md' }` 作为示例，这样它就会在 `https://example.com/license.md` 路径下提供服务。有关更多信息，请参见 [`build.license`](/config/build-options#build-license) 文档。
...
如果你更倾向于将所有的 CSS 抽取到一个文件中，你可以通过设置 [`build.cssCodeSplit`](/config/build-options#build-csscodesplit) 为 `false` 来禁用 CSS 代码分割。
