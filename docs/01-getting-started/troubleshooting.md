...
date: "2025-11-16"
published: true
excerpt: "> 你还可以查看 Rollup 的故障排除指南 了解更多。"
---
...
或者，如果服务器在 VS Code devcontainer 中运行，请求可能会出现停滞。要修复此问题，请参阅 [Dev Containers / VS Code Port Forwarding](#dev-containers-vs-code-port-forwarding)。
...
> Server responded with status code 431. See [https://vite.dev/guide/troubleshooting.html#\_431-request-header-fields-too-large](https://vite.dev/guide/troubleshooting.html#_431-request-header-fields-too-large).
...
如果你正在使用开发容器或 VS Code 的端口转发功能，可能需要在配置中将 [`server.host`](/config/server-options#server-host) 选项设置为 `127.0.0.1` 才能使其正常工作。
...
如果你正在 WSL2 中运行 Vite，Vite 无法在某些场景下监听文件变化。请查看 [`server.watch` 选项](/config/server-options#server-watch) 的描述。
...
*   **实现优雅的回退**: 实现优雅的回退实现动态导入的错误处理，以便在缺少块时重新加载页面。请参阅[加载错误处理](./build#load-error-handling)以了解更多详细信息。
...
可以通过修改 [`build.rollupOptions.output.chunkFileNames`](./../config/build-options#build-rollupoptions) 中的块文件名来绕过此问题，因为这些扩展程序通常会根据文件名（例如包含 `ad` 或 `track` 的文件名）来阻止请求。
...
Node.js 调试器将在根文件夹中生成 `vite-profile-0.cpuprofile` 文件，前往 [https://www.speedscope.app/](https://www.speedscope.app/) ，点击 `BROWSE` 按钮上传 CPU 性能分析文件以检查结果。
