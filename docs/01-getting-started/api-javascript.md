...
date: "2025-11-16"
published: true
excerpt: "Vite 的 JavaScript API 是完全类型化的，我们推荐使用 TypeScript 或者在 VS Code..."
---
...
当使用 [中间件模式](/config/server-options#server-middlewaremode) 与 [WebSocket 代理配置](/config/server-options#server-proxy) 时，父 http 服务器应该在 `middlewareMode` 中提供，以正确绑定代理。
...
**相关内容：** [server.fs.allow](/config/server-options#server-fs-allow)
...
**相关内容：** [`.env` Files](./env-and-mode#env-files)
...
**相关内容：** [路径规范化](./api-plugin#path-normalization)
