...
date: "2025-11-16"
published: true
excerpt: "当需要将应用部署到生产环境时，只需运行  命令。"
---
...
你也可以通过 [`build.target` 配置项](/config/build-options#build-target) 指定构建目标，最低支持 `es2015`。如果设置较低的目标值，Vite 仍然需要这些最低的浏览器支持范围，因为它依赖于[原生的 ESM 动态导入](https://caniuse.com/es6-module-dynamic-import)和 [`import.meta`](https://caniuse.com/mdn-javascript_operators_import_meta)：
...
如果你需要在嵌套的公共路径下部署项目，只需指定 [`base` 配置项](/config/shared-options#base)，然后所有资源的路径都将据此配置重写。这个选项也可以通过命令行参数指定，例如 `vite build --base=/my/public/path/`。
...
当这个库要进行发布构建时，请使用 [`build.lib` 配置项](/config/build-options#build-lib)，以确保将那些你不想打包进库的依赖进行外部化处理，例如 `vue` 或 `react`：

vite.config.js (单入口)vite.config.js (多入口)
...
格式可通过 [`build.lib.format`](/config/build-options#build-lib)选项配置。
...
package.json (单入口)package.json (多入口)
...
如果您的库导入了 CSS，除了内置的 JS 文件外，它还将作为一个 CSS 文件打包在一起，例如 `dist/my-lib.css`。文件名默认为 `build.lib.fileName`，但也可以使用 [`build.lib.cssFileName`](/config/build-options#build-lib)进行更改。
...
在库模式中，所有 [`import.meta.env.*`](./env-and-mode) 的使用都会在构建生产版本时被静态替换。但是，`process.env.*` 的使用不会，这样你的库的使用者就可以动态地改变它。如果这是不可取的，你可以使用 `define: { 'process.env.NODE_ENV': '"production"' }` 来静态替换它们，或者使用 [`esm-env`](https://github.com/benmccann/esm-env) 来更好地兼容打包工具和运行时。
...
*   拷贝的 [公共文件](./assets#the-public-directory)
